...
def _client_with_kb(make_mock, mock_embedder, turns=None):
    ...
    traj = TrajectoryStore(":memory:")
    reg = ToolRegistry()
    # 照 assembly.py:151 原样注册全局 search_knowledge（用默认 collection="knowledge"，即
...
def test_uploaded_doc_is_searchable_from_chat(make_mock, mock_embedder, tool_turn, text_turn):
    """回归（端到端）：上传文档 → 聊天里模型调 search_knowledge → 必须能检索到。

    此前 search_knowledge 用默认 collection="knowledge"（无冒号 → owner 判成 "_global"），
    ...
    client = _client_with_kb(
        ...
        turns=[tool_turn("search_knowledge", '{"query": "光合作用"}'),
    ...
    assert finished, "模型应调用了 search_knowledge"
...
def test_chat_search_knowledge_does_not_leak_across_users(make_mock, mock_embedder,
                                                       tool_turn, text_turn):
    """隔离的另一半：搜到的只能是自己的。修 collection 时若图省事写成无冒号，
    两个用户会共用 _global，等于互相看得见对方的资料。"""
    client = _client_with_kb(
        make_mock, mock_embedder,
        turns=[tool_turn("search_knowledge", '{"query": "机密"}'), text_turn("没找到。")])
    # alice 上传
