...
# 重开/换考意图：考试进行中若用户明确要「重新考/换一套/再来一套/考考我」，应结束当前场、放行让模型
# 重新开考，而不是把这句话当作对当前题的作答吞掉（否则残留的旧考试会让新考试永远起不来）。用词
# 刻意收窄到几乎不可能是作答的措辞，避免把正常作答误判成重开。
RESTART_INTENT_RE = re.compile(
    r"重新考|重新出题|重新开始考|换一?套|换一?批|换一?组|换一?题|"
    r"再来一(套|组|批|次|轮)|开始新的?考试|考考我")
