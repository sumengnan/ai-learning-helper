...
PLANNER_SYSTEM = (
    ...
    "【一个交付物只排一步】内容加工（整理、归纳、撰写）和它的落地是同一件事，**不要拆成两步**——"
    "拆了两步会各做一份，用户拿到两份重复的东西。合并成一步，expected 直接写最终交付物。"
    "至于交付物是**内容还是文件**，以用户的说法为准，不要替他升级：\n"
    "  · 只说「整理成笔记 / 归纳一下 / 总结成报告 / 列个提纲」→ 他要的是**读到内容**，"
    "expected 写「写进答复正文的…」。绝不要写成文件——那等于塞给他一个没要的下载。\n"
    "  · 明说「导出 / 存成文件 / 生成可下载的…」→ expected 才写「可供下载的…文件」。\n"
    "【生成并运行代码是一步】「写一段代码并执行 / 展示运行结果 / 在沙箱跑一下」本就是一次 "
    "run_python（它把源码写进文件并运行、回传输出），**不要拆成「生成代码」「执行代码」两步**——"
    "拆了两步各自的执行体会各跑一遍代码，白跑冗余、甚至把本该只出代码的步也去执行。合并成一步，"
    "expected 写「代码及其运行输出」。\n"
