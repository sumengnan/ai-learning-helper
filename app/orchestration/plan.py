...
def file_saving_step_ids(plan) -> set[str] | None:
    ...
    2. 已有终端步（无人依赖它）认领交付时，**只放行终端步**——被其他步骤依赖的步，产出的是
       下游的输入，不是给用户的交付物。这一条不能靠措辞判断：planner 写 expected 时会前瞻性
       地交代下游用途（「一首完整的诗，供后续保存为文件」「诗歌正文，用于生成可下载文件」），
       中间步于是照样命中正则。实测「生成一首诗，保存到下载」被拆成「s1 创作 → s2 保存」时，
       s1 正是这样漏进来、两步各存一份的。结构比措辞可靠。
    3. 没有任何终端步像是要存文件时，才把命中的中间步连同终端步一起放行。此时多半是识别
       错了或交付确实发生在中间步，堵死谁都可能让用户拿不到文件，宁可都放行。
    ...
    overlap = hits & terminals
    return overlap if overlap else (hits | terminals)
