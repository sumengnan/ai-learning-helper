...
- set_log_context(**fields)：把 run_id/conv_id 等塞进 contextvar，之后同一异步任务内
  每条日志自动带上 `[run_id=.. conv_id=..]` 前缀，便于把一次请求从头到尾串起来看。
"""
...
# 当前执行上下文的关联串（空串=无）。contextvars 天然随 await/子任务传播。
_log_ctx: ContextVar[str] = ContextVar("log_ctx", default="")
