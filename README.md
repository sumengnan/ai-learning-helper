...
- **模型分档**:三档。规划用主模型(判断质量要求高);终局 review 与单步质检 `Critic.validate`
  都走 judge 档(`HARNESS_JUDGE_MODEL`,未配则回退主模型)——用独立/更强模型当裁判可降低"自己
  给自己打高分"的偏差,且恒定关思考。单步 validate 上调到 judge 是因为它能判 impossible
  (终结该步并抑制重规划),一次误判代价放大到整条任务分支,故不再图快用便宜档;代价是它每子步
  都跑,走 judge 会加每轮延迟。triage / 执行子步走快速档(`HARNESS_FAST_MODEL`,未配则回退
